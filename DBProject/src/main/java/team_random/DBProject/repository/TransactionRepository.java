...
public interface TransactionRepository extends JpaRepository<Transaction, Integer> {
    ...
    @Query(value = "SELECT DISTINCT p.name, sum(t.num) FROM transactions t, products p WHERE t.pid = p.id group by p.name ORDER BY sum(t.num) DESC ",nativeQuery = true)
    List<Map<String,Integer>> groupTransByName();

    @Query(value = "SELECT DISTINCT p.name,sum(t.num*p.price) FROM transactions t, products p " +
            "WHERE t.pid = p.id group by p.name ORDER BY sum(t.num*p.price) DESC ",nativeQuery = true)
    List<Map<String,Integer>> sortTransByProfits();

    @Query(value = "SELECT p.name,sum(t.num) FROM transactions t, products p, salespersons sp, stores s " +
            "WHERE t.pid = p.id AND p.salesperson_id = sp.id AND sp.sid = s.id AND s.rid = ?1 GROUP BY p.name ORDER BY sum(t.num) DESC ",nativeQuery = true)
    List<Map<String,Integer>> groupTransByNameInRegion(int region_id);

    @Query(value = "SELECT p.name,sum(t.num*p.price) FROM transactions t, products p, salespersons sp, stores s " +
            "WHERE t.pid = p.id AND p.salesperson_id = sp.id AND sp.sid = s.id AND s.rid = ?1 GROUP BY p.name ORDER BY sum(t.num*p.price) DESC ",nativeQuery = true)
    List<Map<String,Integer>> sortTransByProfitsInRegion(int region_id);

    @Query(value = "SELECT p.name,sum(t.num) FROM transactions t, products p, salespersons sp " +
            "WHERE t.pid = p.id AND p.salesperson_id = sp.id AND sp.sid = s.id GROUP BY p.name ORDER BY sum(t.num) DESC ", nativeQuery = true)
    List<Map<String,Integer>> groupTransByNameInStore(int store_id);

    @Query(value = "SELECT p.name,sum(t.num*p.price) FROM transactions t, products p, salespersons sp " +
            "WHERE t.pid = p.id AND p.salesperson_id = sp.id AND sp.sid = s.id GROUP BY p.name ORDER BY sum(t.num*p.price) DESC ", nativeQuery = true)
    List<Map<String,Integer>> sortTransByProfitsInStore(int store_id);
}